...
Before #187 merges, use its 0.8.0 breaking window to mark `ltk_meta`'s public error enums non-exhaustive, so the streaming work (#192, `docs/design/bin-streaming.md` §13) can add variants in minor releases.
...
- [x] Decision recorded for `PropertyPathErrorKind` / `ResolveErrorKind` / `PatchError`: all four gained the attribute (design doc §15.5)
