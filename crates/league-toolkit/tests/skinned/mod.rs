...
use league_toolkit::core::primitives::{Sphere, AABB};
...
pub fn read() {
    ...
    assert_eq!(
        mesh.aabb(),
        AABB::new(
            vec3(-11.59685, 0.16613889, -5.102246),
            vec3(11.607941, 29.03124, 10.6147995)
        )
    );
    assert_eq!(
        mesh.bounding_sphere(),
        Sphere::new(vec3(0.0055451393, 14.59869, 2.7562768), 20.116425)
    );
    assert_eq!(
        mesh.ranges(),
        [SkinnedMeshRange::new("lambert2", 0, 573, 0, 2067)]
    );
