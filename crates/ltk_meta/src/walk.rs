...
//! the owned tree (`&PropertyValueEnum<M>`) and the streaming view ([`RawValue`]) both
...
/// (`V = &PropertyValueEnum<M>`) and over the view (`V = RawValue<'a, M>`) alike.
///
...
impl<R: io::Read + io::Seek, M: Default> ObjectStream<'_, R, M> {
    ...
    where
        E: From<Error>,
        W: for<'a> Visitor<'a, RawValue<'a, M>, Error = E>,
    {
