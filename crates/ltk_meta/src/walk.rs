...
//! The trail borrows the tree and allocates nothing per segment. A visitor renders it for a
//! node it reports on and for nothing else.
//!
...
///
/// Beside the segments the trail keeps the **class context**: for each `Field` segment, the
/// class hash of the node the field was read on. A name table lookup takes that hash.
#[derive(Debug)]
pub struct Trail<V> {
