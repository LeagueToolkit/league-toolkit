...
Implemented: [section 4](#s4) to [section 9](#s9) - reading, writing, the path language,
resolution and apply - and [section 15](#s15), ritobin text, short of byte parity with moonshadow's
printer, tracked as #242. Designed and unimplemented: [section 10](#s10) to [section 14](#s14)
(merge, `ValuePath`, diff, join, the per-record surface), tracked as #219 to #223. `ValuePath`
itself, and the walk that produces one, are specified in `value-walk.md`.
